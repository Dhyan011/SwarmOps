  ...
  if (!patch) {
    return (
      <div className="glass-card-static p-6 text-center">
        <p className="text-base text-slate-200 font-medium">No code diff available</p>
      </div>
    );
  }
        ...
        <pre className="text-[15px] leading-6 font-mono p-0 m-0">
          {lines.map((line) => (
            <div
              ...
              `}
            >
              <span className="w-10 text-right pr-4 text-slate-200 select-none shrink-0 text-base leading-6">
                {line.number}
              </span>
              <span
                className={`
